		...
		if (entry.type === "damage-group") {
			for (const subEntry of entry.entries) {
				...
				if (type === "") {
					...
					if (item?.system.actionType === "heal")
						type = "healing";
				}
...
export let processCreateBetterRollsMessage = (message, user) => {
	...
	//@ts-ignore
	if (debugEnabled > 1)
		debug("process preCreateBetterRollsCard ", message, installedModules["betterrolls5e"], message.content?.startsWith('<div class="dnd5e red-full chat-card"'));
	let actorId = brFlags.actorId;
	let tokenId = brFlags.tokenId;
	if (tokenId && !tokenId.startsWith("Scene")) { // remove when BR passes a uuid instead of constructed id.
		const parts = tokenId.split(".");
	...
	if (message.speaker) {
		...
		if (!message.speaker?.scene)
			message.update({ "speaker.scene": canvas?.scene?.id });
		//@ts-ignore speaker, update
		if (!message.speaker?.token && tokenId)
			message.update({ "speaker.token": tokenId });
	}
	let damageList = [];
	let otherDamageList = [];
	...
	if (!workflow)
		workflow = new BetterRollsWorkflow(actor, item, message.speaker, targets, null);
...
export function checkOverTimeSaves(message, data, options, user) {
	if (!message.rolls?.length || !["skill", "save", "check"].includes(data.flags?.dnd5e?.roll?.type))
		return true;
...
export function betterRollsButtons(message, html, data) {
	if (!message.flags.betterrolls5e)
		return;
	//@ts-ignore speaker
...
export function addChatDamageButtonsToHTML(totalDamage, damageList, html, actorId, itemUuid, tag = "damage", toMatch = ".dice-total", style = "margin: 0px;") {
	...
	// adding a quarter damage button?
